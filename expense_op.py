...
def save_expense(cursor, db):
    ...
    with st.form(key='expense_submit_form', clear_on_submit=False, border=True):
        ...
        number_of_items = st.number_input('Number Of Items*')
        amount = st.text_input('Amount*')
        payment_methods = st.selectbox('Select Payment Method*', payment_methods)
    ...
    if st.session_state.flag:
        ...
        with st.form(key='final', clear_on_submit=True, border=True):
            ...
            if st.form_submit_button('Are you Sure?'):
                ...
                query = '''Insert into expense (expense_date, category, number_of_items, amount, payment_methods,
                                                notes, documents) 
                        VALUES (%s, %s, %s, %s, %s)
                        '''
                values = (expense_date, category, number_of_items, amount, payment_methods, notes, str(all_documents))
    ...
    df = pd.read_sql('''SELECT id, expense_date, category,number_of_items, amount,payment_methods, notes, documents FROM expense''', con=db)
